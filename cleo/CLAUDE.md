...
## 12. 查今日行程走 calendar_today_recap recipe(CLE-476)

「今天有什麼行程?」「今天我還有什麼行程?」仍由 scope_classifier 判 `calendar_query`,
但 `INTENT_TO_RECIPE["calendar_query"] = "calendar_today_recap"` 後,launcher 會先看
`CLEO_CALENDAR_QUERY_USE_RECIPE`。flag 預設 **off** 時直接 fall through 舊
`CalendarQueryHandler`;flag on 才啟動 `calendar_today_recap` recipe,方便 1 秒回退。

新 recipe 是**唯讀單發**:`calendar.today_recap` action 透過 `ApiCalendarTodayReader`
重用既有 `calendar_today`(今日 bounds、已過期過濾、進行中保留、nearest-first),再輸出
`RenderDirective(kind="schedule_list")`。Discord 端用 `schedule_list` embed,無 View/按鈕;
LINE 只吃 plain-text fallback。metadata 先保留 `items[].event_id`,但本版不掛編輯/刪除。

結尾關心句由 `LLMPersonaCareWriter` 走 `LLMOrchestrator.call(...)` 一次,system prompt 來自
`PersonaEngine.build_system_prompt(task="query")`,user prompt 注入近 30 turns 的
`ConversationMemoryService.recent(limit=30)` 與已過濾的今日剩餘行程。prompt 禁止宣稱
已新增/修改/取消/通知任何人,禁「真的耶/對啊/哈哈」和罐頭祝福;LLM 或 memory/persona
依賴失敗時只退化成固定安全關心句,行程卡仍照常送出。Cleo 在這條路只讀行程、只關心,
**不代辦**。
