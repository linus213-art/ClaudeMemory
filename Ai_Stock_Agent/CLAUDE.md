...
- **Log dir**（launchd jobs 寫入）：`/Users/linus/Ai_Stock_Agent/logs/`
  - `api.{out,err}.log`、`daily.{out,err}.log`、`weekly.{out,err}.log`、`kpi.{out,err}.log`、`healthcheck.{log,err.log}`、`datasync_{price,fundamental}.{out,err}.log`、`leading_indicator_{us,jp}.{out,err}.log`、`glossary_unverified.{out,err}.log`、`data_retention.{out,err}.log`、`indicators_pipeline.{out,err}.log`
...
./scripts/restart_all.sh           # postgres → 11 個 aistockagent jobs → API health probe
...
2. **Ai_Stock_Agent 所有 launchd jobs**（`kickstart -k`，並行）：
   ...
   5. `com.linus.aistockagent.data_retention`（每月 1 號 03:00）
   6. `com.linus.aistockagent.datasync_price`（週一～週五 14:30）
   7. `com.linus.aistockagent.datasync_fundamental`（每週六 07:00）
   8. `com.linus.aistockagent.leading_indicator_us`（週日～五 06:00）
   9. `com.linus.aistockagent.leading_indicator_jp`（週一～五）
   10. `com.linus.aistockagent.glossary_unverified`（每週一 08:00）
   11. `com.linus.aistockagent.healthcheck`（每 300 秒，呼 `/healthz`、失敗時 Discord 告警 + 嘗試自動 kickstart API）
   12. `com.linus.aistockagent.indicators_pipeline`（週一～五 14:35；M7 加入，跑 `ai_stock_agent.scripts.run_indicators_pipeline`：refresh `adjusted_ohlcv` → 增量寫 `daily_indicators` → 重算 `daily_signals`。詳見 `docs/Technical_Indicator_Module_README.md` §7）
...
./scripts/launchctl_manage.sh unload    # bootout 所有 11 個 aistockagent.* jobs
...
```bash
./scripts/launchctl_manage.sh load      # 把 launchd/ 下 11 個 plist 安裝到 /Library/LaunchDaemons/ 並 bootstrap
...
**單一 SQL canonical**：`ai_stock_agent/db/schema.sql`（10 張表：`stock`、`price_daily`、`fundamental`、`user_watchlist`、`analysis_result`、`audit_log`、`llm_usage`、`kpi_tracking`、`glossary`、`hidden_link`）。
...
| `scripts/launchctl_manage.sh {load,unload,restart,status}` | 安裝 / 拔除 / 查詢 11 個 launchd jobs |
