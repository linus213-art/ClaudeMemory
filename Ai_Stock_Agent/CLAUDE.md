...
- **出場方法(2026-09)**:`analysis/exit_methods.py` 原本註明「只給發現組」,現已可接進波段/當沖兩組——
  `StrategyOptions.exit_{stop,tp}_method` 兩個都設就**取代** toggle 式出場(兩套同跑會互搶,分不清功勞),
  由 `sim_intraday._method_exit` 執行,判定順序與理由字串與發現組 `_champion_fired` 逐行相同(停損優先),
  兩組的 `exit_method` 才可並排比較。context 建構抽到共用的 `data/exit_context.py`(ATR14/MA5/前高20),
  發現組也改用它 —— 複製一份的話兩邊 ATR 定義會漂移,對照實驗就失去意義。
  **不要照抄 pf4 的 champion**:`sim_exit_eval` 反事實統計顯示 pf4 用的 `trailing` 是**最差**的停利
  (+3.03% vs `fixed` +9.26%),且 pf4 實際期望值是 **−22.8/筆**(勝率 23.8% 的打平門檻 3.20 > 賺賠比 3.11)。
  ⚠️ 跨 family 不可比:tp 只在獲利時觸發、stop 只在虧損時觸發,100% 勝率是結構性的不是 edge。
